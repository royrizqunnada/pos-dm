<x-filament-panels::page>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <p class="mt-1 text-2xl font-bold text-gray-900">Rp {{ number_format($totals['gross'], 0, ',', '.') }}</p>
        </div>
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right font-semibold text-gray-600">Total Terjual</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($this->items as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right text-gray-700">Rp {{ number_format($item->gross, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-primary-600">Rp {{ number_format($item->base_owed, 0, ',', '.') }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-10 text-center text-gray-400">Belum ada penjualan pada tanggal ini.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
