<x-filament-panels::page>
    ...
    <div class="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        @php($kpis = [
            ...
            ['label' => 'Total Uang Masuk', 'value' => rupiah($totals['total_gross']), 'icon' => 'M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z', 'accent' => false],
            ['label' => 'Dibayar ke Vendor', 'value' => rupiah($totals['total_base_owed']), 'icon' => 'M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z', 'accent' => false],
            ['label' => 'Margin Saya (Hari Ini)', 'value' => rupiah($totals['total_margin']), 'icon' => 'M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941', 'accent' => true],
        ])
        @foreach ($kpis as $kpi)
            <div @class([
                ...
                <div class="px-4 py-3.5">
                    ...
                    <div class="mt-3 grid grid-cols-3 gap-2 text-center">
                        ...
                        <div class="rounded-lg bg-gray-50 py-2 dark:bg-white/5">
                            ...
                            <p class="mt-0.5 text-sm font-semibold text-gray-900 dark:text-white">{{ rupiah($row['total_gross']) }}</p>
                        </div>
                    </div>
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-white/5">
                @forelse ($this->rows as $row)
                    <tr class="transition hover:bg-gray-50/70 dark:hover:bg-white/5">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right font-medium text-gray-900 dark:text-white sm:px-5">{{ rupiah($row['total_base_owed']) }}</td>
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right text-gray-700 dark:text-gray-300 sm:px-5">{{ rupiah($row['total_gross']) }}</td>
            ...
            @if ($this->rows->isNotEmpty())
                <tfoot>
                    <tr class="border-t-2 border-gray-200 bg-gray-50 font-bold text-gray-900 dark:border-white/10 dark:bg-white/5 dark:text-white">
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right sm:px-5">{{ rupiah($totals['total_gross']) }}</td>
